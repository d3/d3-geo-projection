...
tape("geoAitoff(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(aitoff([   0,   0]), [480.000000, 250.000000]);
  test.sphericalEqual(aitoff([   0, -90]), [480.000000, 485.619449]);
  test.sphericalEqual(aitoff([   0,  90]), [480.000000,  14.380550]);
  test.sphericalEqual(aitoff([   0, -45]), [480.000000, 367.809724]);
  test.sphericalEqual(aitoff([   0,  45]), [480.000000, 132.190275]);
  test.sphericalEqual(aitoff([-180,   0]), [  8.761101, 250.000000]);
  test.sphericalEqual(aitoff([ 180,   0]), [951.238898, 250.000000]);
  test.sphericalEqual(aitoff([-179,  15]), [ 27.261952, 189.342293]);
  test.sphericalEqual(aitoff([   1,   1]), [482.617728, 247.381972]);
  test.sphericalEqual(aitoff([  45,  87]), [489.158099, 21.6821110]);
  test.end();
});
...
tape("geoAitoff.invert(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(aitoff.invert([489.158099, 21.6821110]), [  45,  87], 1e-5);
  test.end();
});
