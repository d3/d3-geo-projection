...
tape("geoAiry(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(airy([   0,   0]), [480.000000, 250.000000]);
  test.sphericalEqual(airy([ 180, -90]), [480.000000, 457.944154]);
  test.sphericalEqual(airy([ 180,  90]), [480.000000,  42.055845]);
  test.sphericalEqual(airy([   0, -45]), [480.000000, 350.409232]);
  test.sphericalEqual(airy([   0,  45]), [480.000000, 149.590767]);
  test.sphericalEqual(airy([   1,   1]), [482.216112, 247.783550]);
  test.sphericalEqual(airy([  45,  87]), [487.496494,  47.708572]);
  test.end();
});
...
tape("geoAiry.invert(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(airy.invert([487.496494,  47.708572]), [  45,  87], 1e-5);
  test.end();
});
