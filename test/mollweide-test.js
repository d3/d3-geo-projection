...
tape("geoMollweide(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(mollweide([   0,   0]), [480.000000, 250.000000]);
  test.sphericalEqual(mollweide([   0, -90]), [480.000000, 462.132034]);
  test.sphericalEqual(mollweide([   0,  90]), [480.000000,  37.867965]);
  test.sphericalEqual(mollweide([   0, -45]), [480.000000, 375.591020]);
  test.sphericalEqual(mollweide([   0,  45]), [480.000000, 124.408979]);
  test.sphericalEqual(mollweide([-180,   0]), [ 55.735931, 250.000000]);
  test.sphericalEqual(mollweide([ 180,   0]), [904.264068, 250.000000]);
  test.sphericalEqual(mollweide([-179,  15]), [ 67.028260, 206.573390]);
  test.sphericalEqual(mollweide([   1,   1]), [482.356801, 247.092196]);
  test.sphericalEqual(mollweide([  45,  87]), [495.642877,  40.187699]);
  test.end();
});
...
tape("geoMollweide.invert(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(mollweide.invert([480.000000, 462.132034]), [   0, -90], 1e-4);
  ...
  test.sphericalEqual(mollweide.invert([495.642877,  40.187699]), [  45,  87], 1e-5);
  test.end();
});
