...
tape("geoBoggs(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(boggs([   0,   0]), [480.000000, 250.000000]);
  test.sphericalEqual(boggs([   0, -90]), [480.000000, 473.567218]);
  test.sphericalEqual(boggs([   0,  90]), [480.000000,  26.432781]);
  test.sphericalEqual(boggs([   0, -45]), [480.000000, 371.532657]);
  test.sphericalEqual(boggs([   0,  45]), [480.000000, 128.467342]);
  test.sphericalEqual(boggs([-180,   0]), [ 32.864228, 250.000000]);
  test.sphericalEqual(boggs([ 180,   0]), [927.135771, 250.000000]);
  test.sphericalEqual(boggs([-179,  15]), [ 47.500957, 208.708722]);
  test.sphericalEqual(boggs([   1,   1]), [482.483785, 247.240908]);
  test.sphericalEqual(boggs([  45,  87]), [488.857270,  31.512628]);
  test.end();
});
...
tape("geoBoggs.invert(point) returns the expected values", function(test) {
  ...
  test.sphericalEqual(boggs.invert([488.857270,  31.512628]), [  45,  87], 1e-5);
  test.end();
});
